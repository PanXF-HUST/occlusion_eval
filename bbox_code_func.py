...
def context_x(X,line,kp_,sorted_dist,part_num):
    for i in range(part_num):
        if sorted_dist[i][0] == 'head':
            head_w = int(max(2*abs(kp_[1][0]-kp_[2][0]),abs(kp_[3][0]-kp_[4][0])))
            start = int(kp_[0][0]-int(head_w/2)-X)
            for t in range(head_w+1):
                if line[start+t] == 0:
                    line[start+t]=1
        if sorted_dist[i][0] == 'shoulder':
            start = int(min(kp_[6][0],kp_[7][0]) - X)
            len = int(abs(kp_[6][0] - kp_[7][0]))
            for t in range(len+1):
                if line[start + t] == 0:
                    line[start + t] = 2
        if sorted_dist[i][0] == 'left_elbow':
            start = int(min(kp_[5][0],kp_[7][0]) - X)
            len = int(abs(kp_[5][0]-kp_[7][0]))
            for t in range(len+1):
                if line[start + t] == 0:
                    line[start + t] = 3
        if sorted_dist[i][0] == 'right_elbow':
            start = int (min(kp_[6][0],kp_[8][0]) - X)
            len = int(abs(kp_[6][0]-kp_[8][0]))
            for t in range(len+1):
                if line[start + t] == 0:
                    line[start + t] = 4
        if sorted_dist[i][0] == 'left_arm':
            start = int (min(kp_[7][0],kp_[9][0]) - X)
            len = int (abs(kp_[7][0]-kp_[9][0]))
            for t in range(len+1):
                if line[start + t] == 0:
                    line[start + t] = 5
        if sorted_dist[i][0] == 'right_arm':
            start = int (min(kp_[8][0],kp_[10][0]) - X)
            len = int (abs(kp_[8][0]-kp_[10][0]))
            for t in range(len+1):
                if line[start + t] == 0:
                    line[start + t] = 6
        if sorted_dist[i][0] == 'hip':
            start = int (min(kp_[11][0],kp_[12][0]) - X)
            len = int (abs(kp_[11][0] - kp_[12][0]))
            for t in range(len+1):
                if line[start + t] == 0:
                    line[start + t] = 7
        #身体的部分比较复杂，四个点确定
        if sorted_dist[i][0] == 'body':
            Xs = (kp_[5]+kp_[6])/2
            Xh =  (kp_[11]+kp_[12])/2
            len = int (abs(Xs[0]-Xh[0]))
            start = int(min(Xs[0],Xh[0])-X)
            for t in range(len):
                if line[start + t] == 0:
                    line[start + t] = 8
        if sorted_dist[i][0] == 'left_knee':
            start = int (min(kp_[11][0], kp_[13][0]) - X)
            len = int (abs(kp_[11][0] - kp_[13][0]))
            for t in range(len + 1):
                if line[start + t] == 0:
                    line[start + t] = 9
        if sorted_dist[i][0] == 'right_knee':
            start = int (min(kp_[12][0], kp_[14][0]) - X)
            len = int (abs(kp_[12][0] - kp_[14][0]))
            for t in range(len + 1):
                if line[start + t] == 0:
                    line[start + t] = 10
        if sorted_dist[i][0] == 'left_ankle':
            start = int (min(kp_[13][0], kp_[15][0]) - X)
            len = int (abs(kp_[13][0] - kp_[15][0]))
            for t in range(len + 1):
                if line[start + t] == 0:
                    line[start + t] = 11
        if sorted_dist[i][0] == 'right_ankle':
            start = int (min(kp_[14][0], kp_[16][0]) - X)
            len = int (abs(kp_[14][0] - kp_[16][0]))
            for t in range(len + 1):
                if line[start + t] == 0:
                    line[start + t] = 12
    return line
...
def context_y(Y,line,kp_,sorted_dist,part_num):
   ...
   for i in range(part_num):
        ...
        if sorted_dist[i][0] == 'head':
            # head_h = max(2*abs(kp_[1][0]-kp_[2][0]),abs(kp_[3][0]-kp_[4][0]))
            head_h = int (abs(kp_[0][1] - (kp_[5][1]+kp_[6][1])/2))
            start = int (kp_[0][1]-int(head_h/2)-Y)
            for t in range(head_h+1):
                if line[start+t] == 0:
                    line[start+t]=1
        if sorted_dist[i][0] == 'shoulder':
            start = int (min(kp_[6][1],kp_[7][1]) - Y)
            len = int (abs(kp_[6][1] - kp_[7][1]))
            for t in range(len+1):
                if line[start + t] == 0:
                    line[start + t] = 2
        if sorted_dist[i][0] == 'left_elbow':
            start = int (min(kp_[5][1],kp_[7][1]) - Y)
            len = int (abs(kp_[5][1]-kp_[7][1]))
            for t in range(len+1):
                if line[start + t] == 0:
                    line[start + t] = 3
        if sorted_dist[i][0] == 'right_elbow':
            start = int (min(kp_[6][1],kp_[8][1]) - Y)
            len = int (abs(kp_[6][1]-kp_[8][1]))
            for t in range(len+1):
                if line[start + t] == 0:
                    line[start + t] = 4
        if sorted_dist[i][0] == 'left_arm':
            start = int (min(kp_[7][1],kp_[9][1]) - Y)
            len = int (abs(kp_[7][1]-kp_[9][1]))
            for t in range(len+1):
                if line[start + t] == 0:
                    line[start + t] = 5
        if sorted_dist[i][0] == 'right_arm':
            start = int (min(kp_[8][1],kp_[10][1]) - Y)
            len = int (abs(kp_[8][1]-kp_[10][1]))
            for t in range(len+1):
                if line[start + t] == 0:
                    line[start + t] = 6
        if sorted_dist[i][0] == 'hip':
            start =int (min(kp_[11][1],kp_[12][1]) - Y)
            len = int (abs(kp_[11][1] - kp_[12][1]))
            for t in range(len+1):
                if line[start + t] == 0:
                    line[start + t] = 7
        #身体的部分比较复杂，四个点确定
        if sorted_dist[i][0] == 'body':
            Xs = (kp_[5]+kp_[6])/2
            Xh = (kp_[11]+kp_[12])/2
            len = int (abs(Xs[1]-Xh[1]))
            # len = int(round(abs(Xs[1].item()-Xh[1].item()),0))
            start = int (min(Xs[1],Xh[1]) - Y)

            for t in range(len+1):
                if line[start + t] == 0:
                    line[start + t] = 8
        if sorted_dist[i][0] == 'left_knee':
            start = int (min(kp_[11][1], kp_[13][1]) - Y)
            len = int (abs(kp_[11][1] - kp_[13][1]))
            for t in range(len + 1):
                if line[start + t] == 0:
                    line[start + t] = 9
        if sorted_dist[i][0] == 'right_knee':
            start = int (min(kp_[12][1], kp_[14][1]) - Y)
            len = int (abs(kp_[12][1] - kp_[14][1]))
            for t in range(len + 1):
                if line[start + t] == 0:
                    line[start + t] = 10
        if sorted_dist[i][0] == 'left_ankle':
            start = int (min(kp_[13][1], kp_[15][1]) - Y)
            len = int (abs(kp_[13][1] - kp_[15][1]))
            for t in range(len + 1):
                if line[start + t] == 0:
                    line[start + t] = 11
        if sorted_dist[i][0] == 'right_ankle':
            start = int (min(kp_[14][1], kp_[16][1]) - Y)
            len = int (abs(kp_[14][1] - kp_[16][1]))
            for t in range(len + 1):
                if line[start + t] == 0:
                    line[start + t] = 12
    return line
