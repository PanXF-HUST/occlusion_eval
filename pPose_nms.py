...
def pose_nms(bboxes, bbox_scores, pose_preds, pose_scores):
    ...
    ori_bboxes = bboxes.clone()           #20200313 pan edit
    ...
    widths = xmax - xmin
    heights = ymax - ymin
    ref_dists = alpha * np.maximum(widths, heights)#参考距离???

    nsamples = bboxes.shape[0]#应该是？？？有多少个人
    human_scores = pose_scores.mean(dim=1)#对一个人的整体的平均分，由各个部位的分数平均求得

    human_ids = np.arange(nsamples)#生成一个行人id的索引
    # Do pPose-NMS
    pick = []
    merge_ids = []
    while(human_scores.shape[0] != 0):
        ...
        simi = get_parametric_distance(pick_id, pose_preds, pose_scores, ref_dist)  #参数距离
        ...
        bboxes = np.delete(bboxes,delete_ids,axis=0)           # 20200313,pan edit


    # print('after delete,the bbox is __________________')
    # print(bboxes)
    ...
    bboxes_pick = ori_bboxes[pick]              #20200313 pan edit
    ...
    for j in range(len(pick)):
        ...
        # merge_bboxes = ori_bboxes[merge_id]
    ...
    # print(final_result)         # pan edit 20200313
    #result is a dict which contain keypoints ,kpscore,proposal_score,human_boxes,bboxes_score,eval_score
...
def get_parametric_distance(i, all_preds, keypoint_scores, ref_dist):
    '''
    Input:
    i  the num.th of the all num
    all_preds           pose_preds      (n,17,2)
    keypoint_scores     pose_scores     (n,17,1)
    ref_dist            reference_distance  constant

    Output:
    final_dist          parametric_distance number
    '''
...
def write_json(all_results, outputpath, for_eval=False):
    ...
    for im_res in all_results:
        ...
        for human in im_res['result']:
            keypoints = []
            #result = {'image_id':None,'category_id':None,'keypoints':None,'score':None}
            result = {}
