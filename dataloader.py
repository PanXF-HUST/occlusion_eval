...
''' the function is writen by panda  '''
from occlusion_eval import occlud_eval
from bbox_code_func import bbox_code_process
    ...
    def update(self):
        ...
        for i in range(self.datalen):
            ...
            with torch.no_grad():
                ...
                inps, pt1, pt2 = crop_from_dets(inp, boxes, inps, pt1, pt2, im_name)
...
def crop_from_dets(img, boxes, inps, pt1, pt2,im_name):
    '''
    ...
    imght = img.size(1)
    imgwidth = img.size(2)
    # cv2.imwrite(os.path.join(opt.outputpath, 'before', '1.jpg'), img)
    # cv2.imwrite('examples/det/before', img)
    tmp_img = img
    tmp_img[0].add_(-0.406)
    tmp_img[1].add_(-0.457)
    tmp_img[2].add_(-0.480)
    # cv2.imwrite(os.path.join(opt.outputpath, 'after', '2.jpg'), tmp_img)
    # cv2.imwrite('examples/det/after', tmp_img)
    ...
    for i, box in enumerate(boxes):
        ...
        img = np.transpose(inps[i].numpy() * 255 * 1.5, (1, 2, 0))
        img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        img_name = './human/' + im_name[5:17] + str(i) + '.jpg'
        cv2.imwrite(img_name, img)

    # 返回经过映射的输入图像tensor类型以及经过尺度变换的左上角和右下角坐标
    return inps, pt1, pt2
